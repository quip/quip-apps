...
import {RecordEntity} from "../../base-field-builder/model/record.js";
import {
    parseFieldsData,
    parseFieldValue,
...
export function fieldSupportedFn(field) {
    ...
    if (field.dataType === "TextArea") {
        return (
            field.extraTypeInfo === "PlainTextArea" ||
            field.extraTypeInfo === "RichTextArea"
        );
    }
...
export class SalesforceRecordEntity extends RecordEntity {
    ...
    save() {
        ...
        this.getFields().forEach(field => {
            if (field.isDirty()) {
                // Format date, number etc in case onBlur is not called
                if (field.format) {
                    field.format();
                }
                if (!field.isValid()) {
    ...
    initFields_(isCreation, initialFieldsOverride = null) {
        ...
        if (initFieldsMap[this.getType()]) {
            initFieldsP = Promise.resolve(initFieldsMap[this.getType()]);
        } else {
            // TODO(nvo/jesseditson): Preferably Salesforce Record should not
            // be aware that the field is an object with apiName key, but rather
            // it should expect fields to be list of strings.
            initFieldsP = this.getClient()
                .fetchLayoutFields(this.getType(), false)
                .then(fields => fields.map(f => f.apiName))
                .catch(e => {
                    return [];
                });
        }

        return initFieldsP.then(initFields => {
            if (initFields.length === 0) {
                initFields.push(this.getHeaderField());
            }
            initFields.map(fieldKey => {
