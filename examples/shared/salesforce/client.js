...
} from "../base-field-builder/error.js";
import {
    parseFieldsData,
    parseListViews,
    parseLayout,
    ...
    parseSchema,
    parseSoqlRecords,
} from "../base-field-builder/response-handler.js";
import {
    fieldSupportedFn,
...
const LISTS_ENDPOINT = "ui-api/list-ui";
...
const DASHBOARDS_ENDPOINT = "analytics/dashboards";
 ...
 * @const
 */
const HTML_ESCAPE_ALL_RE = /['"_%\\]/;
/**
 * @type {RegExp}
 * @const
 */
const HTML_ESCAPE_SC_RE = /'/g;
/**
 * @type {RegExp}
 * @const
 */
const HTML_ESCAPE_DC_RE = /"/g;
/**
 * @type {RegExp}
 * @const
 */
const HTML_ESCAPE_US_RE = /_/g;
/**
 * @type {RegExp}
 * @const
 */
const HTML_ESCAPE_PE_RE = /%/g;
/**
 * @type {RegExp}
 * @const
 */
const HTML_ESCAPE_BS_RE = /\\/g;
...
export class SalesforceClient {
    ...
    fetchLists(listType) {
        const listsUrl = `${this.apiUrl_}/${LISTS_ENDPOINT}/${listType}`;
        return this.request_("GET", listsUrl).then(response =>
            Object.values(response.lists)
        );
    }
    ...
    updateListData(records) {
        return Promise.all(
            records.map(({recordId, fields}) =>
                this.request_(
                    "PATCH",
                    `${this.apiUrl_}/${RECORDS_ENDPOINT}/${recordId}`,
                    {fields})
            ));
    }
    ...
    fetchPicklistOptions(recordType, recordTypeId, fieldName) {
        const url =
            `${
                this.apiUrl_
            }/${OBJECT_INFO_ENDPOINT}/${recordType}/picklist-values/${recordTypeId}` +
            (fieldName ? `/${fieldName}` : "");
        return this.request_("GET", url).then(response => {
            if (fieldName) {
                return parsePicklistOptions(response);
            }
            return response.picklistFieldValues;
        });
    }
    ...
    fetchObjectTypes() {
        ...
        return this.request_("GET", objectInfoUrl).then(response =>
            Object.values(response.objects).sort(({label: a}, {label: b}) =>
                a === b ? 0 : a > b ? 1 : -1
            )
        );
    }
    ...
    fetchRecordDataForListView(
        ...
        searchTerm,
        searchField) {
        if (searchTerm.length >= 3 && listViewId === "All") {
            return this.fetchAutoSuggestQuery_(
                recordType,
                searchTerm,
                200).then(response => this.parseAutoSuggestRecords_(response));
        } else if (listViewId !== "All" && listViewId !== "RecentlyViewed") {
            return this.fetchListViewsQuery_(recordType, listViewId)
                .then(response => this.parseListViewsRecords_(response))
                .then(records =>
                    this.filterListViewsRecords_(records, searchTerm)
                );
        } else {
    ...
    fetchAutoSuggestQuery_(recordType, searchTerm, limit) {
        ...
        return this.request_(
            "GET",
            this.apiUrl_ + "/search/suggestions",
            searchQueryParams);
    }

    parseAutoSuggestRecords_(response) {
        if (!response.autoSuggestResults) {
            throw response;
        }

        return response.autoSuggestResults.map(record => {
            return this.getSuggestionFromRecord_(record);
        });
    }

    fetchListViewsQuery_(recordType, listViewId) {
        const resultsUrl = `/sobjects/${recordType}/listviews/${listViewId}/results`;
        return this.request_("GET", this.apiUrl_ + resultsUrl);
    }

    parseListViewsRecords_(response) {
        if (!response.records) {
            throw response;
        }

        return response.records.map(record =>
            this.getListViewsSuggestionFromRecord_(record)
        );
    }

    getListViewsSuggestionFromRecord_(record) {
        if (!record.columns) {
            throw record;
        }

        const nameColumn = record.columns.find(column => {
            return column.fieldNameOrPath === "Title";
        });
        const recordName = (nameColumn && nameColumn.value) || "";
        const idColumn = record.columns.find(column => {
            return column.fieldNameOrPath === "Id";
        });
        const recordId = (idColumn && idColumn.value) || "";
        return {name: recordName, id: recordId};
    }

    filterListViewsRecords_(records, searchTerm) {
        return records.filter(record =>
            record.name.toLowerCase().includes(searchTerm.toLowerCase())
        );
    }

    getSuggestionFromRecord_(record) {
        const recordId = record["Id"] || "";
        const recordName = record["Name"] || record["CaseNumber"] || recordId;
        return {name: recordName, id: recordId};
    }
    ...
    reformatQuery_(query, recordType, searchTerm, searchField) {
        ...
        searchTerm = this.htmlEscape_(searchTerm);
    ...
    // This is for Salesforce SOQL query string escape only.
    // More details can be found in
    // https://developer.salesforce.com/docs/atlas.en-us.soql_sosl.meta/soql_sosl/sforce_api_calls_soql_select_quotedstringescapes.htm
    htmlEscape_(searchTerm) {
        if (!searchTerm || !HTML_ESCAPE_ALL_RE.test(searchTerm)) {
            return searchTerm;
        }
        // Replace backslash with two backslashes
        if (searchTerm.indexOf("\\") != -1) {
            searchTerm = searchTerm.replace(HTML_ESCAPE_BS_RE, "\\\\");
        }
        if (searchTerm.indexOf("'") != -1) {
            searchTerm = searchTerm.replace(HTML_ESCAPE_SC_RE, "\\'");
        }
        if (searchTerm.indexOf('"') != -1) {
            searchTerm = searchTerm.replace(HTML_ESCAPE_DC_RE, '\\"');
        }
        if (searchTerm.indexOf("_") != -1) {
            searchTerm = searchTerm.replace(HTML_ESCAPE_US_RE, "\\_");
        }
        if (searchTerm.indexOf("%") != -1) {
            searchTerm = searchTerm.replace(HTML_ESCAPE_PE_RE, "\\%");
        }
        return searchTerm;
    }
    ...
    fetchRecentDashboards() {
        const url = `${this.apiUrl_}/${DASHBOARDS_ENDPOINT}`;
        return this.request_("GET", url);
    }
    ...
    fetchDashboard(dashboardId) {
        const url = `${
            this.apiUrl_
        }/${DASHBOARDS_ENDPOINT}/${dashboardId}/describe`;
        return this.request_("GET", url);
    }
